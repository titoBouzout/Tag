...
class Tag():
	...
	def __init__(self):
		...
		Tag.regexp_self_closing_optional 	= re.compile("^<?\?|\!|area|base|br|col|frame|hr|img|input|link|meta|param|command|embed|source|li|p", re.I);
		Tag.regexp_self_closing 					= re.compile("^<?\?|\!|area|base|br|col|frame|hr|img|input|link|meta|param|command|embed|source", re.I);
	...
	def is_self_closing(self, content, ignore_optional = True):
		if ignore_optional:
			...
			return Tag.regexp_self_closing_optional.match(content)
	...
	def name(self, content, ignore_optional = True):
		...
		if self.is_valid(tag_name) and not self.is_self_closing(tag_name, ignore_optional):
			return tag_name
		else:
